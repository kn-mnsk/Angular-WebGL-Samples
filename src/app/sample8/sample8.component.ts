...
export class Sample8Component extends WebGLBase {
  ...
  override initialize(): void {
    ...
    this.shader.loadProgram(this.gl, './app/shaders/sample8.vs', './app/shaders/sample8.fs');
  ...
  override initializeGlBuffers(): void {
    ...
    this.texture.asyncLoadVideo("./assets/textures/video/Firefox.mp4", this.video)
      .then(() => {
        this.texture.checkCopyVideo(() => {
          // console.log(`Debug  ${this.title}Component saying Finished asysncLoadVideo`);
          this.texture.updateVideoTexture(this.gl, this.texture0, this.video);
        });
      })
      .catch(error => {
        throw new Error(`${this.title}Component: Error in asyncLoadVideo - ${error}`);
      }
      );
  ...
  override render(): void {
    ...
    // console.log(`Debug  ${this.title}Component: Rendering in process`);
    ...
    // this.gl.uniform1i(this.gl.getUniformLocation(this.shader.getProgram, "uSampler"), 0);

    if (this.texture.copiedVideo) {
      this.texture.updateVideoTexture(this.gl, this.texture0, this.video);
    }
